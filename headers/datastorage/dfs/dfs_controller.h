...
public:
  ...
  // Internal use only
  std::string addLocalFile(const Actor<KeyPrivate> &actor,
                           const std::filesystem::path &filePath,
                           std::string targetVirtualFilePath,
                           DFS::Encryption securityLevel);
  void removeLocalFile(const Actor<KeyPrivate> &actor,
  ...
  std::string permissionFilePath(const std::string &actorId,
                                 const std::string &hashFile);

private:
  bool insertDataChunk(std::string data, uint64_t position,
                       std::filesystem::path file);
  bool removeDataChunk(uint64_t position, uint64_t length,
                       std::filesystem::path file);
  DBRow makeActrDirDBRow(std::string fileHash, std::string fileHashPrev,
                         std::string filePath, uint64_t fileSize);
  uint64_t
  calculateSizeTaken(const std::string &folder = DFS::Basic::fsActrRoot);
...
public:
  ...
  void addDirData(const ActorId &actorId,
                  const std::vector<DFS::Packets::DirRow> &dirRows);
...
signals:
  void added(ActorId actorId, std::string fileHash, std::string visual,
             int64_t size);
