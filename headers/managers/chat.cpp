...
Chat::Chat(QByteArray chatId, QByteArray key, QByteArray currentSession,
           AccountController* accountController, QByteArray ownerId) {
...
QByteArray Chat::getCurrentSession() {
  if (this->_currentSession != "-1") return this->_currentSession;
...
QByteArray Chat::findCurrentSession() {
  ...
  do {
    currentSession++;
    file.setFileName(chatStore + this->_chatId + "/" +
                     currentSession.toByteArray());
  } while (file.exists());
  currentSession--;
  return currentSession.toByteArray();
}
QByteArray Chat::createNewSession(QByteArray key, QByteArray sessionNumb) {
  if (!SaveSession((chatStore + this->_chatId + "/" + "currentSession"),
                   sessionNumb))
    return "-1";
  if (!SaveSession((keyStore + this->_chatId + "/" + "currentSession"),
                   sessionNumb))
    return "-1";
...
QByteArray Chat::getChatPrivateKey() {
  return _accountController->getCurrentActor().getKey()->decrypt(
      unloadChatKey());
}
...
bool Chat::SaveSession(QByteArray sessionPath, QByteArray sessionNumb) {
  ...
  qDebug() << "[Warning] Error when write sessionpath" << sessionPath
           << " sessionnumb " << sessionNumb;
  return false;
}

void Chat::sendMessage(QByteArray message) {
  emit sendMessageToChat(this->_chatId, getCurrentSession(),
                         _accountController->getCurrentActor().getId(),
...
QByteArray Chat::getChatId() const { return this->_chatId; }

QByteArray Chat::getEncryptionKey() const { return this->_encryptionKey; }

QByteArray Chat::getSession() const { return this->_currentSession; }

AccountController* Chat::getAccountController() const {
  return this->_accountController;
}

void Chat::InviteNewUser(QByteArray inviterId, QByteArray inviterSign,
...
bool Chat::isUserVerify(QByteArray actorId) {
  ...
  if (file.open(QIODevice::ReadOnly)) {
    ...
    if (!_accountController->getActor(BigNumber(list.at(0)))
             .getKey()
             ->verify(list.at(0), list.at(1)))
      return false;
...
Chat::~Chat() { delete _accountController; }
