...
namespace json_convert {
    ...
    template <typename T>
    struct has_custom_magic<T,
                            std::void_t<decltype(magic::custom_magic<T>::read(std::declval<T>())),
                                        decltype(magic::custom_magic<T>::write(std::declval<std::string>()))>>
        : std::true_type { };
...
struct fmt::formatter<
    T,
    std::enable_if_t<boost::describe::has_describe_members<T>::value || json_convert::has_custom_magic_v<T>,
                     char>> {
