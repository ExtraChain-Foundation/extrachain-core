...
class EXTRACHAIN_EXPORT DfsService {
public:
    using FileEvent     = ExtraChain::Core::Event<const ActorId &, const Dfs::DirRow &>;
    using FileIdEvent   = ExtraChain::Core::Event<const ActorId &, const std::string &>;
    using ProgressEvent = ExtraChain::Core::Event<const ActorId &, const std::string &, int>;
    using CollectionEvent =
        ExtraChain::Core::Event<const ActorId &, const Dfs::DirRow &, const HistoricalCollectionRow &>;
    using VectorRowEvent = ExtraChain::Core::Event<const ActorId &, const Dfs::DirRow &, const DbRow &>;
...
private:
    ...
    FileEvent                                       updated_event_;
    FileIdEvent                                     removed_event_;
    FileIdEvent                                     local_removed_event_;
    FileEvent                                       uploaded_event_;
    ProgressEvent                                   upload_progress_event_;
    FileEvent                                       downloaded_event_;
    ProgressEvent                                   download_progress_event_;
    FileEvent                                       wait_downloaded_event_;
    ExtraChain::Core::Event<>                       collection_downloaded_event_;
    CollectionEvent                                 collection_changed_event_;
    VectorRowEvent                                  vector_row_added_event_;
    VectorRowEvent                                  vector_row_removed_event_;
...
public:
    ...
    [[nodiscard]] FileEvent                 &stored_event() noexcept;
    [[nodiscard]] FileEvent                 &added_event() noexcept;
    [[nodiscard]] FileEvent                 &updated_event() noexcept;
    [[nodiscard]] FileIdEvent               &removed_event() noexcept;
    [[nodiscard]] FileIdEvent               &local_removed_event() noexcept;
    [[nodiscard]] FileEvent                 &uploaded_event() noexcept;
    [[nodiscard]] ProgressEvent             &upload_progress_event() noexcept;
    [[nodiscard]] FileEvent                 &downloaded_event() noexcept;
    [[nodiscard]] ProgressEvent             &download_progress_event() noexcept;
    [[nodiscard]] FileEvent                 &wait_downloaded_event() noexcept;
    [[nodiscard]] ExtraChain::Core::Event<> &collection_downloaded_event() noexcept;
    [[nodiscard]] CollectionEvent           &collection_changed_event() noexcept;
    [[nodiscard]] VectorRowEvent            &vector_row_added_event() noexcept;
    [[nodiscard]] VectorRowEvent            &vector_row_removed_event() noexcept;

    void notify_stored(const ActorId &owner_id, const Dfs::DirRow &dir_row);
    void notify_added(const ActorId &owner_id, const Dfs::DirRow &dir_row);
    void notify_updated(const ActorId &owner_id, const Dfs::DirRow &dir_row);
    void notify_removed(const ActorId &owner_id, const std::string &file_id);
    void notify_local_removed(const ActorId &owner_id, const std::string &file_id);
    void notify_uploaded(const ActorId &owner_id, const Dfs::DirRow &dir_row);
    void notify_upload_progress(const ActorId &owner_id, const std::string &file_id, int progress);
    void notify_downloaded(const ActorId &owner_id, const Dfs::DirRow &dir_row);
    void notify_download_progress(const ActorId &owner_id, const std::string &file_id, int progress);
    void notify_wait_downloaded(const ActorId &owner_id, const Dfs::DirRow &dir_row);
    void notify_collection_downloaded();
    void notify_collection_changed(const ActorId                 &owner_id,
                                   const Dfs::DirRow             &dir_row,
                                   const HistoricalCollectionRow &historical_row);
    void notify_vector_row_added(const ActorId &owner_id, const Dfs::DirRow &dir_row, const DbRow &row);
    void notify_vector_row_removed(const ActorId &owner_id, const Dfs::DirRow &dir_row, const DbRow &row);
...
private:
    ...
    std::mutex                                   files_waiting_mutex_;
