...
BASE_PORT="${2:-17840}"
BUILD_DIR="${EXTRACHAIN_TEST_BUILD:-$HOME/ExC/Messenger/testnet/core-integ/tests/build}"
...
SOAK_PID=$!
trap 'kill "$SOAK_PID" 2>/dev/null; pkill -f "extrachain-node-run join $WORK" 2>/dev/null' EXIT
...
( cd "$WORK/joiner" && exec "$BUILD_DIR/extrachain-node-run" join data 127.0.0.1 "$TARGET" \
      $((BASE_PORT + 60)) $((BASE_PORT + 20)) ) > "$WORK/joiner.log" 2>&1 &
JOIN_PID=$!
...
if [ "$verdict" = "joined" ] && [ "$soak_rc" -eq 0 ]; then
    echo "LIVE-JOIN PASS: fresh node caught the moving tip at $TARGET while the committee kept finalizing"
