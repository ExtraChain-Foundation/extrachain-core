...
void DfsController::initializeActor(const ActorId &actorId) {
  ...
  std::filesystem::create_directories(DFS::Basic::fsActrRoot + pathDelim +
                                      actorId.toStdString());
  DBConnector actrDirFile =
      DFS::Tables::ActorDirFile::actorDbConnector(actorId.toStdString());
...
std::string DfsController::addLocalFile(const Actor<KeyPrivate> &actor,
...
#ifdef ANDROID
  auto tempPath =
      "dfs/temp" + QString::number(QRandomGenerator::global()->bounded(1000) +
  ...
  std::filesystem::path placeInDFS =
      DFS::Basic::fsActrRootW + DFS::Basic::separator +
      actor.id().toString().toStdWString() + DFS::Basic::separator;
  std::filesystem::path dfsPath =
      DFS::Path::filePath(actor.id().toStdString(), fileHash);
  if (std::filesystem::exists(dfsPath) &&
      std::filesystem::file_size(dfsPath) == fileSize) {
  ...
  auto actrDirFile =
      DFS::Tables::ActorDirFile::actorDbConnector(actor.id().toStdString());
  auto lastFileHash = DFS::Tables::ActorDirFile::getLastHash(actrDirFile);
  const DBRow rowData =
      makeActrDirDBRow(msg.FileHash, lastFileHash, msg.Path, msg.Size);
  ...
  dirsFile.replace(DFS::Tables::DirsFile::TableName,
                   {{"actorId", actor.id().toStdString()},
                    {"lastModified", rowData.at("lastModified")}});

  // add permission
  PermissionManager permissionManager(&node);
  DFS::Permission::PermissionMode mode =
      DFS::Permission::PermissionMode(DFS::Permission::Permission::Service);
...
void DfsController::removeLocalFile(const Actor<KeyPrivate> &actor,
                                    const std::string &filePath) {
  std::string fileHash = Utils::calcKeccakForFile(filePath); // TODO: get hash
  DFS::Packets::RemoveFileMessage msg = {.Actor = actor.id().toStdString(),
                                         .FileHash = fileHash};
  PermissionManager permissionManager(&node);
  DFS::Permission::RemovePermission rmPermission = {
      .actor = actor.id().toStdString(), .fileHash = fileHash};
  permissionManager.remove(rmPermission);
  node.network()->send_message(msg, MessageType::DfsRemoveFile);
}

void DfsController::updatePermissions(
    const std::vector<DFS::Permission::UpdatePermission> &permissions) {
  PermissionManager permissionManager(&node);
  for (const auto &permission : permissions) {
    ...
    case DFS::Permission::Action::AddAction: {
      ...
      node.network()->send_message(newPermission,
                                   MessageType::DfsAddPermission);
      break;
    }

    case DFS::Permission::Action::RemoveAction: {
      DFS::Permission::RemovePermission rmPermission = {
          .actor = permission.actor, .fileHash = permission.fileHash};
      permissionManager.remove(rmPermission);
      break;
    }
...
std::string DfsController::addFile(const DFS::Packets::AddFileMessage &msg,
                                   bool loadBytes) {
  std::string pathDelim = Utils::platformDelimeter();
  std::string actrDirFilePath = DFS::Basic::fsActrRoot + pathDelim + msg.Actor +
                                pathDelim + DFS::Basic::fsMapName;
  ...
  if (loadBytes) {
    if (msg.Size >= m_bytesLimit - m_sizeTaken) {
      ...
      DFS::Packets::RequestFileSegmentMessage reqMessage = {.Actor = msg.Actor,
                                                            .FileHash =
                                                                msg.FileHash,
                                                            .Path = msg.Path,
                                                            .Offset = 0};
...
std::string DfsController::getFileFromStorage(ActorId owner,
                                              std::string fileHash) {
  Actor<KeyPrivate> localOwner =
      node.accountController()->currentProfile().getActorConst(owner);
  std::string pathDelim = Utils::platformDelimeter();
  std::filesystem::path realFilePath = DFS::Basic::fsActrRoot + pathDelim +
                                       owner.toStdString() + pathDelim +
                                       fileHash;
  DBConnector actrDirFile;
  std::string actrDirFilePath = DFS::Basic::fsActrRoot + pathDelim +
                                owner.toStdString() + pathDelim +
                                DFS::Basic::fsMapName;
  if (!actrDirFile.open(actrDirFilePath)) {
  ...
  std::vector<DBRow> actrDirData =
      DFS::Tables::ActorDirFile::getFileDataByHash(&actrDirFile, fileHash);
...
bool DfsController::removeFile(const DFS::Packets::RemoveFileMessage &msg) {
  ...
  std::vector<DBRow> actrDirData =
      DFS::Tables::ActorDirFile::getFileDataByHash(&actrDirFile, msg.FileHash);
  std::string prevHash;
  for (auto it = actrDirData.begin(); it < actrDirData.end(); it++) {
    if (it->at("fileHash") == msg.FileHash) {
      ...
      if (!std::filesystem::remove(realFilePath)) {
        qDebug() << "File removal by path " << realFilePath.c_str()
                 << " failed";
        return false;
      }
    }
    if ((it->at("fileHashPrev") == msg.FileHash) && (!prevHash.empty())) {
      actrDirFile.update("UPDATE " + DFS::Tables::ActorDirFile::TableName +
                         " SET fileHashPrev = " + "'" + prevHash + "' " +
                         "WHERE " + "fileHash = " + "'" + it->at("fileHash") +
                         "'");
    }
  }
...
std::string DfsController::permissionFilePath(const std::string &actorId,
...
std::string
DfsController::insertFragment(const DFS::Packets::EditSegmentMessage &msg) {
  ...
  if (actrDirData.size() > 2) {
    qDebug() << "[Dfs] editFile: Query select failed: Query result has "
                "unsupported size:"
             << actrDirData.size();
    qFatal("Error 4");
    return "";
  ...
  for (auto it = actrDirData.begin(); it < actrDirData.end(); it++) {
    if (it->at("fileHash") == msg.FileHash) {
      ...
      // actrDirFile.update("UPDATE " + DFS::Tables::ActorDirFile::TableName + "
      // SET fileSize = " + "'"
      //                    + std::to_string(newFileSize) + "' " + "WHERE " +
      //                    "fileHash = " + "'"
      //                    + it->at("fileHash") + "'");
    }
...
bool DfsController::insertDataChunk(std::string data, uint64_t position,
                                    std::filesystem::path file) {
  std::string pathDelim = Utils::platformDelimeter();
  std::filesystem::path tempFilePath =
      "temp" + pathDelim + file.stem().string();
  ...
  boost::interprocess::file_mapping fmapSource(file.c_str(),
  ...
  std::size_t i = 0;
  for (i = position; i < fz;
       i = i + DFS::Basic::sectionSize) { // copy old data to new temp file
    if (i + DFS::Basic::sectionSize < fz) {
      boost::interprocess::mapped_region rightRegion(
          fmapSource, boost::interprocess::read_write, i,
  ...
  std::ofstream ofsres(file.c_str(),
                       std::ios::out | std::ios::app | std::ios::binary);
  boost::interprocess::file_mapping fmapTarget(tempFilePath.c_str(),
                                               boost::interprocess::read_write);
  uint64_t fzres = std::filesystem::file_size(tempFilePath);

  for (i = 0; i < fzres;
       i = i + DFS::Basic::sectionSize) { // copy new data to old file
    if (i + DFS::Basic::sectionSize < fzres) {
      boost::interprocess::mapped_region rightRegion(
          fmapTarget, boost::interprocess::read_write, i,
          DFS::Basic::sectionSize);
      char *rr_ptr = static_cast<char *>(rightRegion.get_address());
...
bool DfsController::removeDataChunk(uint64_t position, uint64_t length,
                                    std::filesystem::path file) {
  std::string pathDelim = Utils::platformDelimeter();
  std::filesystem::path tempFilePath =
      "temp" + pathDelim + file.stem().string();
...
DBRow DfsController::makeActrDirDBRow(std::string fileHash,
...
std::string
DfsController::extractFragment(boost::interprocess::file_mapping &fmapTarget,
                               uint64_t offset, uint64_t fragmentSize) {
  boost::interprocess::mapped_region rightRegion(
      fmapTarget, boost::interprocess::read_only, offset, fragmentSize);
  char *rr_ptr = static_cast<char *>(rightRegion.get_address());
...
DfsController::extractFragment(boost::interprocess::file_mapping &fmapTarget,
                               uint64_t offset) {
  boost::interprocess::mapped_region rightRegion(
      fmapTarget, boost::interprocess::read_only, offset);
  char *rr_ptr = static_cast<char *>(rightRegion.get_address());
  return std::string(rr_ptr, rightRegion.get_size());
}

void DfsController::requestSync() {
  node.network()->send_message(Utils::currentDateSecs(),
...
void DfsController::sendSync(uint64_t lastModified,
  ...
  DBConnector dirsFile(DFS::Basic::dirsPath);
  auto actors = dirsFile.select(
      "SELECT actorId FROM " + DFS::Tables::DirsFile::TableName +
...
void DfsController::requestDirData(const ActorId &actorId) {
  node.network()->send_message(actorId, MessageType::DfsDirData,
                               MessageStatus::Request);
}
...
void DfsController::sendDirData(const ActorId &actorId, uint64_t lastModified,
  ...
  if (!std::filesystem::exists(
          DFS::Tables::ActorDirFile::actorDbPath(actorId.toStdString()))) {
    return;
  }
  ...
  if (!rows.empty()) {
    node.network()->send_message(
        std::pair{actorId, rows}, MessageType::DfsDirData,
        MessageStatus::Response, messageId, Config::Net::TypeSend::Focused);
  }
}

void DfsController::addDirData(
    const ActorId &actorId, const std::vector<DFS::Packets::DirRow> &dirRows) {
  bool res =
      DFS::Tables::ActorDirFile::addDirRows(actorId.toStdString(), dirRows);
...
void DfsController::requestFile(const ActorId &actorId,
                                const std::string &fileHash) {
  std::filesystem::remove(DFS::Path::filePath(actorId.toStdString(), fileHash));
  node.network()->send_message(std::pair{actorId, fileHash},
                               MessageType::DfsRequestFile,
                               MessageStatus::Request);
}

void DfsController::sendFile(const ActorId &actorId,
                             const std::string &fileHash,
                             const std::string &messageId) {
  if (fileHash.empty()) {
  ...
  auto dirRow =
      DFS::Tables::ActorDirFile::getDirRow(actorId.toStdString(), fileHash);
  DFS::Packets::AddFileMessage msg = {.Actor = actorId.toStdString(),
                                      .FileHash = dirRow.fileHash,
                                      .Path = dirRow.filePath,
                                      .Size = dirRow.fileSize};
  qDebug() << "sendFile" << actorId.toString() << dirRow.fileHash.c_str()
           << dirRow.filePath.c_str() << dirRow.fileSize;
...
std::string
DfsController::sendFragment(const DFS::Packets::RequestFileSegmentMessage &msg,
                            const std::string &messageId) {
  std::filesystem::path realFilePath =
      DFS::Path::filePath(msg.Actor, msg.FileHash);
  if (!std::filesystem::exists(realFilePath)) {
    return "";
    qFatal("[Dfs] No file");
  }

  boost::interprocess::file_mapping fmapTarget(realFilePath.c_str(),
  ...
  DFS::Packets::EditSegmentMessage fragment = {.Actor = msg.Actor,
  ...
  emit uploadProgress(msg.Actor, msg.FileHash,
                      double(msg.Offset) / double(fileSize) * 100);
  return "";
}

std::string
DfsController::addFragment(const DFS::Packets::EditSegmentMessage &msg) {
  ...
  DBConnector actrDirFile =
      DFS::Tables::ActorDirFile::actorDbConnector(msg.Actor);
  ...
  std::vector<DBRow> actrDirData =
      DFS::Tables::ActorDirFile::getFileDataByHash(&actrDirFile, msg.FileHash);
  actrDirData = actrDirFile.select("SELECT * FROM " +
                                   DFS::Tables::ActorDirFile::TableName +
  ...
  emit downloadProgress(msg.Actor, msg.FileHash,
                        double(msg.Offset) / double(fileSize) * 100);
  DFS::Packets::RequestFileSegmentMessage reqMessage = {.Actor = msg.Actor,
                                                        .FileHash =
                                                            msg.FileHash,
...
std::string
DfsController::deleteFragment(const DFS::Packets::DeleteSegmentMessage &msg) {
  std::string pathDelim = Utils::platformDelimeter();
  DBConnector actrDirFile;
  std::string actrDirFilePath = DFS::Basic::fsActrRoot + pathDelim + msg.Actor +
                                pathDelim + DFS::Basic::fsMapName;
...
uint64_t DfsController::bytesLimit() const { return m_bytesLimit; }
