...
PermissionManager::Permission PermissionManager::getHighestPermission(const QString &userId, const QString &fileHash)
{
...
PermissionManager::Permission PermissionManager::getPermission(const Actor<KeyPrivate> &actor, const GetPermissionMsg & msg)
{
    ...
    if(permFilePermission == Read || permFilePermission == Write ||
       permFilePermission == Delete || permFilePermission == Edit)
    {
    ...
    qDebug() << __FUNCTION__ << " Actor " << actor.idStd().c_str() << " has no permission to read the Permissions file";
...
bool PermissionManager::setPermission(const Actor<KeyPrivate> &actor, const SetPermissionMsg & msg)
{
    ...
    if(newPermission == Permission::NoPermission)
    {
        ...
        {
            qDebug() << __FUNCTION__ << " Actor " << actor.idStd().c_str() << " has no permission to change the Permissions file";
            return false;
        }
    ...
    else    // Update existing entry
    {
        ...
        const std::string & updateQuery = (std::stringstream ()
            << "UPDATE " << Config::DataStorage::permissionTable
            << " SET permission = '" << newPermissionStr
            << "', signature = '" << newSignatureStr
            << "' WHERE fileHash = '" << fileHashStr
            << "' AND userId = '" << userIdStr
            << "' AND signature = '" << signatureStr << "'").str();
...
PermissionManager::Permission PermissionManager::getUserPermission(const QString &userId, const QString &fileHash)
{
    DBRow currentPermissionRow = findDBRow(userId, fileHash);
    ...
    if( currentPermission != Permission::Read &&
        currentPermission != Permission::Write &&
        currentPermission != Permission::Delete &&
        currentPermission != Permission::Edit)
    {
...
QString PermissionManager::makeActorDirPath(const Actor<KeyPrivate> &actor)
{
    return FileSystem::pathConcat(
                FileSystem::pathConcat(QCoreApplication::applicationDirPath(), RootDir),
                actor.id().toString());
}
...
DBRow PermissionManager::makeDBRow(const QString & fileHash, const Permission permission, const QString & userId, const QString & signature) {
    return {
...
DBRow PermissionManager::findDBRow(const QString & userId, const QString & fileHash){
    const std::string selectQuery = (std::stringstream()
                                     << "SELECT * FROM " << Config::DataStorage::permissionTable
                                     << " WHERE fileHash = '" << fileHash.toStdString()
                                     << "' AND userId = '" << userId.toStdString() << "'").str();
