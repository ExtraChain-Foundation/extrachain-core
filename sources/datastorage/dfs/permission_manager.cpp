...
CriticalErrors PermissionManager::initPermission(const std::string &userId, const std::string &fileHash,
...
CriticalErrors PermissionManager::updatePermission(const DFS::Permission::UpdatePermission &permissionData) {
    const std::string pathToPermissionFile =
    ...
    const auto result =
        std::filesystem::is_empty(pathToPermissionFile) ? CriticalErrors::NoUpdated : CriticalErrors::NoError;
...
CriticalErrors PermissionManager::savePermission(const DFS::Permission::AddPermission &permissionData) {
    ...
    return std::filesystem::is_empty(pathToPermissionFile) ? CriticalErrors::NoSaved
                                                           : CriticalErrors::NoError;
}

CriticalErrors PermissionManager::savePermission(const std::string &userId, const std::string &fileHash,
...
CriticalErrors PermissionManager::rename(const std::string &oldFileHash,
...
CriticalErrors PermissionManager::remove(const DFS::Permission::RemovePermission &rmPermission) {
    const std::string pathToPermissionFile =
        makePermissionFileName(rmPermission.actor, rmPermission.fileHash);
    if (!std::filesystem::remove(pathToPermissionFile))
...
std::vector<std::string> PermissionManager::searchFile(const std::string &dirPath,
    ...
    for (auto &pathToFile : std::filesystem::recursive_directory_iterator(dirPath)) {
        if (std::filesystem::is_regular_file(pathToFile)
            && pathToFile.path().extension() == DFS::Permission::permissionFileExtension) {
...
void PermissionManager::serializePermissionData(const std::string data, uint64_t position,
    ...
    qDebug() << "actor: "
             << QString::fromStdString(getDataByValue(permission, DFS::Permission::PermissionValue::Actor));
    qDebug() << "path: "
             << QString::fromStdString(getDataByValue(permission, DFS::Permission::PermissionValue::Path));
    qDebug() << "userId: "
             << QString::fromStdString(getDataByValue(permission, DFS::Permission::PermissionValue::UserID));
    qDebug() << "file_hash: "
             << QString::fromStdString(
                    getDataByValue(permission, DFS::Permission::PermissionValue::FileHash));
    qDebug() << "signature: "
             << QString::fromStdString(
                    getDataByValue(permission, DFS::Permission::PermissionValue::Signature));
}
...
std::string PermissionManager::getDataByValue(const AddPermission &addPermission, const int &value) {
    ...
    const auto sizeOfValue = [&]() {
        switch (value) {
        case PermissionValue::Actor:
            return addPermission.actor.size();
        case PermissionValue::Path:
            return addPermission.path.size();
        case PermissionValue::UserID:
            return addPermission.userId.size();
        case PermissionValue::FileHash:
            return addPermission.fileHash.size();
        case PermissionValue::Signature:
            return addPermission.signature.size();
        case PermissionValue::Value:
            return sizeof(addPermission.permissionValue);
        }
        return size_t(0);
    };
...
FileDataObject PermissionManager::makeFileData(const std::string &actor, const std::string &path,
    ...
    return { { "actor", actor },       { "path", path },
             { "fileHash", fileHash }, { "permission_value", std::to_string(permissionMode.toInt()) },
             { "userId", userId },     { "signature", signature } };
}

std::string PermissionManager::makePermissionFileName(const std::string &actorId,
                                                      const std::string &hashFile) {
    std::stringstream stringStream;
    stringStream << DFS::Permission::rootDir << "/" << actorId << "/" << hashFile
                 << DFS::Permission::permissionFileExtension;
    return stringStream.str();
}

std::string PermissionManager::makeTempFileName(const std::string &hashFile) {
    std::stringstream stringStream;
    stringStream << DFS::Permission::tempDir << "/"
                 << "tmp_" << hashFile << DFS::Permission::permissionFileExtension;
    return stringStream.str();
}
...
QJsonDocument PermissionManager::makePermissionJsonDocument(const std::string &userId,
                                                            const std::string &fileHash,
                                                            PermissionMode &permissionMode) {
    QJsonObject permissionObject;
    ...
    permissionObject[DFS::Permission::permissionValue.c_str()] =
        std::to_string(permissionMode.toInt()).c_str();
