...
bool HistoricalChain::remove(DFSP::EditSegmentMessage msg) {
    ...
    if (msg.Data == lastSegment.Data) {
        ...
        const bool updated =
            chainFile.update(fmt::format("UPDATE {} SET prevNum={} WHERE hash={}", DFSHC::TableNameHC,
                                         dbRow.at(PREV_NUM), nextRow.at(HASH)));
...
bool HistoricalChain::update(DFSP::EditSegmentMessage msg, const int &num) {
    ...
    updated = chainFile.update(fmt::format("UPDATE {} SET type = {} data = {} hash = {} WHERE data= {}",
                                           DFSHC::TableNameHC, std::to_string(msg.ActionType), msg.Data,
                                           msg.FileHash, editableSegmentMessage.Data));
    return updated;
}
...
DBRow HistoricalChain::getNextRow(const int &currentNum) {
    std::vector<DBRow> res;
    std::pair<DBRow, DBRow> ret;
    std::string GetNextQuery = fmt::format("SELECT * FROM {} WHERE num>{} ORDER BY num DESC LIMIT 1",
                                           DFSHC::TableNameHC, std::to_string(currentNum));
