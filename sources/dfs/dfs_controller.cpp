...
DfsController::DfsController(ExtraChainNode *node)
    : QObject(node)
    , node(node)
    , dirs_manager_(new DirsManager(node))
    ...
    connect(dirs_manager_, &DirsManager::convertion_started, this, [=, this] {
        qDebug() << "convertion_started_dfs_controller";
        emit convertion_begin();
    });

    // connect(&dirs_manager_, &DirsManager::convertion_started, this, &DfsController::convertion_started);
    // dirs_manager_->old_dfs_to_new_dfs_converter();
