...
namespace Pack {
    ...
    std::optional<PackId> Registry::parse_pack_filename(const std::filesystem::path &p) {
        ...
        if (ec != std::errc {} || ptr != stem.data() + stem.size())
            return std::nullopt;
        return id;
    }
    ...
    void Registry::rescan() {
        ...
        std::sort(discovered.begin(), discovered.end(), [](const PackMeta &a, const PackMeta &b) {
    ...
    Reader *Registry::acquire_reader_locked(PackId id) {
        ...
        auto [inserted_it, _] = readers_.emplace(id, std::make_unique<Reader>(std::move(*r)));
        lru_.push_front(id);
    ...
    std::expected<void, Error> Registry::create_pack(PackId                                  pack_id,
        ...
        PackMeta m { .id = pack_id, .first = r->first_section(), .last = r->last_section() };
        {
            ...
            meta_.erase(std::remove_if(meta_.begin(),
                                       meta_.end(),
                                       [&](const PackMeta &x) {
                                           return x.id == pack_id;
                                       }),
                        meta_.end());
            meta_.push_back(m);
            std::sort(meta_.begin(), meta_.end(), [](const PackMeta &a, const PackMeta &b) {
                return a.first < b.first;
            });
        }
        return {};
    ...
    std::vector<Registry::Span> Registry::spans() const {
        ...
        for (const auto &m : meta_)
            out.push_back(Span { .id = m.id, .first = m.first, .last = m.last });
        return out;
    }
    ...
    void Registry::discard_incoming(PackId id) {
        auto incoming = pack_path(id);
        incoming += ".incoming";
        std::error_code ec;
