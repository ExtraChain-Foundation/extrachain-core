...
struct ChainIndex::Impl {
    ...
    bool open() {
        ...
        return stmt_insert_tx && stmt_delete_section && stmt_find_sent && stmt_find_sent_token && stmt_find_recv
               && stmt_find_recv_token && stmt_row_count && stmt_last_section && stmt_actor_select
               && stmt_actor_insert && stmt_token_select && stmt_token_insert && stmt_actor_blob_by_id
               && stmt_token_blob_by_id && stmt_count_actor_type_since;
    }
...
void ChainIndex::on_section_written(const Section &s) {
    ...
    std::lock_guard<std::recursive_mutex> lock(impl_->db_mutex);
...
std::uint64_t ChainIndex::count_for_actor_by_type_since(const std::string &actor,
    ...
    sqlite3_stmt *statement = impl_->stmt_count_actor_type_since;
