...
namespace {
    ...
    bool verify_dfs_effect_authors(DfsService*                                          dfs,
                                   const ExtraChain::Contracts::PreparedContractChange& change,
...
namespace ExtraChain::Core {
    ...
    void ExtraChainNode::process() {
        ...
        dag_peer_info_timer_     = DeadlineTask::create(runtime_executor(), [this] {
            dagPeerInfoTimerTick();
        });
        const auto periodic_task = [this](std::chrono::milliseconds interval, auto handler) {
            return PeriodicTask::create(runtime_executor(), interval, [this, handler] {
                (this->*handler)();
            });
        };
        reward_timer_ =
            periodic_task(std::chrono::milliseconds(MINING_TIMER_TICK), &ExtraChainNode::timer_reward_request);
            ...
            ExtraChain::Contracts::ContractManager>(std::make_unique<
                                                    ...
                                                    ExtraChain::Contracts::RuntimeTuning {
                                                        ...
                                                        .module_cache_entries =
                                                            runtime_profile() == RuntimeProfile::MobileLight ? 2U
                                                                                                             : 8U,
        ...
        auto retry_contracts = [this](const ActorId&, const Dfs::DirRow& row) {
            if (row.folder == Dfs::Basic::TEMPLATE_CONTRACTS) {
                boost::asio::post(serial_executor(), [this] {
                    dag_->retry_contract_transactions();
                });
            }
        };
        core_connections_.emplace_back(dfs_->added_event().subscribe(retry_contracts));
        core_connections_.emplace_back(dfs_->downloaded_event().subscribe(retry_contracts));
        core_connections_.emplace_back(actor_index_->new_actor_saved_event().subscribe([this](ActorId) {
            boost::asio::post(serial_executor(), [this] {
                dag_->retry_contract_transactions();
            });
        }));
        core_connections_.emplace_back(actor_index_->actor_saved_event().subscribe([this](ActorId) {
            boost::asio::post(serial_executor(), [this] {
                dag_->retry_contract_transactions();
            });
        }));
        ...
        core_connections_.emplace_back(
            dfs_->downloaded_event().subscribe([this](const ActorId& owner_id, const Dfs::DirRow& row) {
                boost::asio::post(serial_executor(), [this, owner_id, row] {
                    if (chat_manager_) {
                        chat_manager_->on_file_downloaded(owner_id, row);
                    }
                });
            }));
        core_connections_.emplace_back(dfs_->vector_row_added_event().subscribe(
            [this](const ActorId& owner_id, const Dfs::DirRow& row, const DbRow& db_row) {
                boost::asio::post(serial_executor(), [this, owner_id, row, db_row] {
                    if (chat_manager_) {
                        chat_manager_->on_vector_row_added(owner_id, row, db_row);
                    }
                });
            }));
        core_connections_.emplace_back(dfs_->vector_row_removed_event().subscribe(
            [this](const ActorId& owner_id, const Dfs::DirRow& row, const DbRow& db_row) {
                boost::asio::post(serial_executor(), [this, owner_id, row, db_row] {
                    if (chat_manager_) {
                        chat_manager_->on_vector_row_removed(owner_id, row, db_row);
                    }
                });
            }));
        ...
        if (runtime_profile() == RuntimeProfile::FullNode) {
            reward_timer_->start();
            info_timer_->start();
    ...
    void ExtraChainNode::release_core() {
        if (core_released_.exchange(true, std::memory_order_acq_rel)) {
            return;
        }
        node_enabled.store(false, std::memory_order_release);
        core_connections_.clear();
        dag_sync_timer_.reset();
        dag_peer_info_timer_.reset();
        backfill_timer_.reset();
        ready_timer_.reset();
        reward_timer_.reset();
        info_timer_.reset();
        ...
        if (dfs_) {
            dfs_->prepare_shutdown();
        }
        if (network_service_) {
            network_service_->prepare_shutdown();
        }
        if (dag_) {
            dag_->stop();
        }
    ...
    void ExtraChainNode::queue_actor_broadcast(Actor<KeyPublic> actor) {
        if (on_serial_executor()) {
            actors_broadcast_.push_back(std::move(actor));
            return;
        }
        boost::asio::post(serial_executor(), [this, actor = std::move(actor)]() mutable {
            actors_broadcast_.push_back(std::move(actor));
        });
    }
    ...
    bool ExtraChainNode::create_token_vector() {
        ...
        auto search_result = Dfs::Tables::DirsFile::ActorSpace::
    ...
    void ExtraChainNode::backfill_token_allocations() {
        ...
        backfill_timer_->schedule_after(std::chrono::seconds(10));
    }
    ...
    bool ExtraChainNode::write_actor_rename(const ActorId& actor_id, const std::string& name) {
        ...
        if (name.empty()) {
            ...
            actor_renamed_event_.publish(actor_id, name);
        } else {
    ...
    void ExtraChainNode::start() {
        if (!started_) {
            ...
            ready_timer_ = DeadlineTask::create(serial_executor(), [this] {
                if (network_service_) {
                    network_service_->start_network();
                }
                eInfo("Your node successfully started");
                ready_event_.publish();
            });
            ready_timer_->schedule_after(std::chrono::milliseconds(10));
        }

        // DFS download ranks: chat-actor vectors -> 2,
        ...
        dfs_->set_download_rank_by_name(network_id(), "Usernames", DfsService::RANK_OTHER_VECTORS);
    ...
    boost::asio::any_io_executor ExtraChainNode::runtime_executor() {
        if (!runtime_) {
    ...
    boost::asio::any_io_executor ExtraChainNode::serial_executor() {
        if (!runtime_) {
            throw std::logic_error("ExtraChain Core is not initialized");
        }
        return *serial_executor_;
    }

    bool ExtraChainNode::on_serial_executor() const noexcept {
        return serial_executor_ && serial_executor_->running_in_this_thread();
    }

    ExtraChain::Core::NetworkRuntime& ExtraChainNode::network_runtime() {
        if (!runtime_) {
            throw std::logic_error("ExtraChain Core is not initialized");
        }
        return *runtime_;
    ...
    ExtraChain::Core::Event<RuntimeActivity>& ExtraChainNode::runtime_activity_event() {
        return runtime_activity_changed_event_;
    }

    Event<>& ExtraChainNode::initialized_event() noexcept {
        return initialized_event_;
    }

    Event<>& ExtraChainNode::ready_event() noexcept {
        return ready_event_;
    }

    Event<const ActorId&, const std::string&>& ExtraChainNode::actor_renamed_event() noexcept {
        return actor_renamed_event_;
    }

    Event<>& ExtraChainNode::actor_renames_loaded_event() noexcept {
        return actor_renames_loaded_event_;
    }

    Event<const ActorId&, const std::string&>& ExtraChainNode::subscription_added_event() noexcept {
        return subscription_added_event_;
    }

    void ExtraChainNode::set_runtime_activity(RuntimeActivity activity) {
        if (!on_serial_executor()) {
            boost::asio::post(*serial_executor_, [this, activity] {
                set_runtime_activity(activity);
            });
            return;
        }
        if (runtime_activity_.exchange(activity, std::memory_order_acq_rel) == activity) {
            return;
        }
        if (runtime_profile() == RuntimeProfile::FullNode) {
        ...
        if (activity == RuntimeActivity::Background) {
            ...
            dagTimerStarting(15000);
        }
        runtime_activity_changed_event_.publish(activity);
    }
    ...
    TransactionProveError ExtraChainNode::validate_contract_transaction(const Transaction& transaction) {
        ...
        if (!metadata.has_value() || metadata->schema != 4 || metadata->kind.empty() || metadata->kind.size() > 64
            ...
            || metadata->revision == 0
            || metadata->transitions.size() >= ExtraChain::Contracts::ContractMaximumCalls
        ...
        if (transaction.type() == TransactionType::ContractCall) {
            ...
            if (metadata->method == "init" || metadata->method == "migrate"
                || metadata->method == "authorize_upgrade" || metadata->version != current.version
                || metadata->revision != previous.revision + 1 || metadata->module_hash != current.module_hash
                || metadata->previous_state_hash != previous.state_hash || metadata->checkpoint != checkpoint_due
    ...
    std::expected<ExtraChain::Contracts::ContractReceipt, ExtraChain::Contracts::ContractFailure> ExtraChainNode::
        ...
        return contract_manager_->query(contract_id.to_string(),
                                        signer.id().to_string(),
                                        method,
                                        arguments,
                                        block);
    ...
    ExtraChain::Contracts::ContractCatalogPage ExtraChainNode::list_contracts(
        ...
        return dag_ == nullptr ? ExtraChain::Contracts::ContractCatalogPage {}
                               : dag_->cache().list_contracts(filter);
    }
    ...
    void ExtraChainNode::selfTxRepeatableAdded(const Transaction& transaction) {
        ...
        if (res) {
            subscription_added_event_.publish(row.owner_id, row.file_id);
        }
    }
    ...
    std::expected<std::string, ImportError> ExtraChainNode::export_profile() {
        if (account_controller_->profile_type() == ProfileType::New) {
            std::ifstream file(account_controller_->profile_seed.filename(), std::ios::binary);
            if (!file) {
    ...
    std::expected<std::string, ImportProfileFileError> ExtraChainNode::import_profile_file(
        ...
        auto from_base64 = Utils::from_base64(file_content);
    ...
    std::expected<Transaction, TransactionError> ExtraChainNode::send_transaction(
        const Transaction&       transaction,
        const Actor<KeyPrivate>& signer) {
        if (transaction.type() == TransactionType::Regular
            && token_manager_->is_contract_token(transaction.token())) {
            auto arguments = token_manager_->transfer_arguments(transaction.token(),
    ...
    #ifdef Q_OS_LINUX
        {
            ...
            if (statm.is_open()) {
                ...
                eLog(
                    ...
                    network_service_->forwarded_messages_size(),
                    dag_->sended_transactions_size(),
                    dag_->failed_transactions_size(),
                    dag_->last_txs_size(),
                    dag_->cached_txs_size(),
                    network_service_->active_connections_count(),
    ...
    void ExtraChainNode::notification_token(std::string os, std::string actor_id, std::string token) {
        if (os.empty() || actor_id.empty() || token.empty())
            return;
        auto network_id = actor_index_->network_id();
        if (network_id.is_zero())
            return;
        ...
        // std::map<std::string, std::string> map = { { "actor", actor_id },
        //                                            { "token", mainKey.encrypt(token,
        //                                            publicKey)
        //                                            }, { "os", mainKey.encrypt(os, publicKey)
        //                                            } };
    ...
    void ExtraChainNode::connect_signals() {
        ...
        core_connections_.emplace_back(dag_->timer_start_event().subscribe([this](int delay_ms) {
            dagTimerStarting(delay_ms);
        }));
        core_connections_.emplace_back(dag_->timer_stop_event().subscribe([this] {
            dagTimerStoping();
        }));

        core_connections_.emplace_back(network_service_->socket_activated_event().subscribe(
...
#ifdef IS_APP_CLIENT
                if (ip == network_service_->first_node()) {
                    dag_->start_check();
                }
#else
                dag_->start_check();
        ...
        core_connections_.emplace_back(network_service_->socket_ready_event().subscribe([this]() {
            dfs_->sendSizeRequestMsg(account_controller_->system_actor().id());
        }));

        core_connections_.emplace_back(actor_index_->first_sync_ended_event().subscribe([this]() {
            boost::asio::post(serial_executor(), [this] {
        ...
        core_connections_.emplace_back(
            dfs_->wait_downloaded_event().subscribe([this](const ActorId&, const Dfs::DirRow& dir_row) {
                if (dir_row.file_id == renames_file_id_waiting_) {
                    actor_renames_loaded_event_.publish();
    ...
    void ExtraChainNode::prepare_folders() {
        ...
        eLog("Working directory: {}", std::filesystem::current_path().string());

        // Version compatibility: 0.15.0
        std::error_code error;
        if (std::filesystem::exists("keystore")) {
            std::filesystem::rename("keystore", Profiles::folder, error);
            if (error) {
                eWarning("Cannot migrate keystore folder: {}", error.message());
            }
        }

        for (const auto& folder :
             { Profiles::folder, ChainConst::TMP_FOLDER, ChainConst::DAG_FOLDER, ChainConst::ACTORS_FOLDER }) {
            error.clear();
            std::filesystem::create_directories(folder, error);
            if (error) {
                throw std::filesystem::filesystem_error("Cannot create ExtraChain data folder", folder, error);
            }
        }
    ...
    void ExtraChainNode::dagTimerStarting(int ms) {
        ...
        if (runtime_activity() == RuntimeActivity::Background) {
            return;
        }
        if (dag_sync_timer_) {
