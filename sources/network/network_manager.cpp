...
NetworkManager::NetworkManager(ExtraChainNode *node, std::uint16_t port)
    ...
    io_running_ = true;
    const unsigned int thread_count = std::max(2u, std::thread::hardware_concurrency() / 2);
        ...
        io_threads_.emplace_back([this]() {
            while (io_running_) {
                try {
                    ...
                    eLog("[NetworkManager] IO thread exception: {}", e.what());
                }
            }
        });
    }
...
void NetworkManager::add_all_services_identifiers_to_message(MessageBody &msg) {
    ...
    for (const auto &service : *connectionsLocked) {
        const std::string &ident = service->identifier();
...
void NetworkManager::setup_service_callbacks(SocketService::Ptr service, bool requestListNodes) {
    service->on_error = [this](SocketService::Ptr svc,
                               Network::SocketServiceError error,
                               const std::string &errorData,
                               const std::string &identifier) {
        QMetaObject::invokeMethod(
            this,
            [this, svc, error, errorData, identifier]() { socket_error(svc, error, errorData, identifier); },
            Qt::QueuedConnection);
    };

    service->on_disconnected = [this](SocketService::Ptr svc) {
        QMetaObject::invokeMethod(this, [this, svc]() { remove_socket_connection(svc); }, Qt::QueuedConnection);
    };

    service->on_activated = [this](SocketService::Ptr svc) {
        QMetaObject::invokeMethod(
            this,
            [this, svc]() {
                check_connections_status();
                emit newSocketActivatedWithParams(svc->ip(), svc->identifier());
                emit newSocketActivated();

                if (svc->mode() == SocketMode::Full && svc->ip() != first_node()) {
                    reconn_.insert({ svc->ip(), {} });
                }
            },
            Qt::QueuedConnection);
    };

    service->on_share_connections = [this](SocketService::Ptr svc, const std::set<SocketService::SocketPair> &connections) {
        (void)svc;
        QMetaObject::invokeMethod(
            this,
            [this, connections]() {
                ...
                if (active_connections_count() >= Network::maxConnections && init_ip != first_node_) {
                ...
                for (const auto &[ip, identifier] : connections) {
                    ...
                    for (const auto &conn_item : *connections_locked) {
                        if (ip == init_ip || identifier == node->node_identifier()
                            || conn_item->identifier() == identifier || conn_item->ip() == ip) {
                            can_connect = false;
                            break;
                        }
                    }
...
void NetworkManager::check_port(const QString     ip,
    ...
    const auto ip_str = ip.toStdString();
    asio::co_spawn(
        *ioc_,
        [this, ip_str, protocol, request, isConstant]() -> VoidTask {
            ...
            QMetaObject::invokeMethod(
                this,
                [this, ip_str, protocol, request, isConstant]() {
...
void NetworkManager::start_network() {
    ...
    try {
        tcp::endpoint endpoint(tcp::v4(), ws_port);
        ...
        asio::co_spawn(
            *ioc_,
            [this]() -> VoidTask { co_await accept_loop(); },
            asio::detached);

        eLog("[WS] Start listening on port: {}", ws_port);
    } catch (const std::exception &e) {
        eLog("[NetworkManager] Can't listen port {}: {}", ws_port, e.what());
    }
}

VoidTask NetworkManager::accept_loop() {
    while (io_running_ && acceptor_ && acceptor_->is_open()) {
        auto [ec, socket] = co_await acceptor_->async_accept(asio::as_tuple(asio::use_awaitable));

        if (ec) {
            if (ec != asio::error::operation_aborted) {
                eLog("[WS] Accept error: {}", ec.message());
            }
            break;
        }

        if (active_connections_count() >= Network::maxConnections && !remove_one_connection()) {
            boost::system::error_code close_ec;
            socket.close(close_ec);
            continue;
        }

        auto service = std::static_pointer_cast<WebSocketService>(WebSocketService::from_accepted(std::move(socket), *ioc_, node));
        service->set_direction(SocketDirection::Incoming);
        setup_service_callbacks(service);

        asio::co_spawn(
            *ioc_,
            [service]() -> VoidTask { co_await service->run(true); },
            asio::detached);
    }
}

VoidTask NetworkManager::do_connect_websocket(const std::string &ip,
                                              uint16_t port,
    ...
    try {
        auto service = std::static_pointer_cast<WebSocketService>(
            co_await WebSocketService::connect(*ioc_, ip, port, node, isConstant, is_light));
        service->set_direction(SocketDirection::Outgoing);
        setup_service_callbacks(service);

        reconnections_to_identifier_->emplace(
            NetworkReconnect { .ip = QString::fromStdString(ip), .port = port, .protocol = Network::Protocol::WebSocket },
            "");

        asio::co_spawn(
            *ioc_,
            [service]() -> VoidTask { co_await service->run(false); },
            asio::detached);
    } catch (const std::exception &e) {
        eLog("[WS] Connection failed to {}:{} - {}", ip, port, e.what());
    }
}
...
void NetworkManager::connect_to_websocket(const QString &ip,
    ...
    asio::co_spawn(
        *ioc_,
        [this, ip = ip.toStdString(), port, requestListNodes, isConstant, is_light]() -> VoidTask {
            co_await do_connect_websocket(ip, port, requestListNodes, isConstant, is_light);
        },
        asio::detached);
}
...
void NetworkManager::send_message_connections(const std::string &serialized_message,
    ...
    if (send_mode == SendMode::NeighboursRandom || send_mode == SendMode::OneNeighbourRandom) {
        ...
        if (send_mode == SendMode::NeighboursRandom && active_identifiers.size() > 3) {
            ...
            for (int index : indexes) {
                active_identifiers[index]->send_message(
                    std::vector<uint8_t>(serialized_message.begin(), serialized_message.end()),
                    priority);
            }
        ...
        if (send_mode == SendMode::OneNeighbourRandom) {
            active_identifiers[indexes[0]]->send_message(
                std::vector<uint8_t>(serialized_message.begin(), serialized_message.end()),
                priority);
        }
    }
    ...
    for (const auto &service : *connections_locked) {
        ...
        if (send_checked) {
            calculate_traffic_->add_bytes_sent(service->ip(), serialized_message.size());
            service->send_message(std::vector<uint8_t>(serialized_message.begin(), serialized_message.end()), priority);
...
void NetworkManager::socket_error(SocketService::Ptr          service,
                                  Network::SocketServiceError error,
                                  const std::string          &errorData,
                                  const std::string          &identifier) {
    const std::string ip = service ? service->ip() : "";
    const auto direction = service ? service->direction() : SocketDirection::Outgoing;
    ...
    if (error == Network::SocketServiceError::IncompatibleNetwork
        ...
        emit connectionError(error,
                             ...
                             QString::fromStdString(errorData));
        return;
    }
