...
void NetworkManager::messageReceived(
    ...
    case MessageType::VPNHandshake: {
        ...
        if (status == MessageStatus::Response) {
            ...
            if (inputMsg.vpnType == VPNType::SERVER || inputMsg.vpnType == VPNType::PROXY) {
                qInfo() << "Response 1";
                if (node.vpnConnectorManager->vpnIsClient) {
                    qInfo() << "Response 1 1";
                    ...
                    outputMsg.publicKeyFile    = node.vpnConnectorManager->vpnFileAddedHash[0];
                    ...
                    std::string messageIdToReturn;
                    {
                        ...
                        auto vpnHandhakeCacheInProccessLocked =
                            *node.vpnConnectorManager->vpnHandhakeCacheInProccess;
            ...
            if (inputMsg.vpnType == VPNType::SERVER) {
                ...
                if (!node.vpnConnectorManager->CheckVPNHandshakeAccess(identifier, 100))
                    return;
                ...
                if (!inputMsg.countryEndpoint.empty()
                    && inputMsg.countryEndpoint
                           != node.vpnConnectorManager->vpnInitPublicIPAndCountry.second.toStdString()) {
                ...
                if (!inputMsg.networkIdentifiersToIgnore.contains(
                    ...
                    if (node.vpnConnectorManager->networkCallback(
                            inputMsg,
                            mb.sender_id,
                        ...
                        node.vpnConnectorManager->networkCallback(
                            inputMsg,
                            mb.sender_id,
                        ...
                        lockedChainIndexAll.insert(
                            output.blockedChainIndexes.begin(),
                            output.blockedChainIndexes.end());
                        ...
                        node.vpnConnectorManager->vpnHandhakeCacheInProccess->emplaceBack(
                            VPNConnectorManager::VPNHandhakeCache { .uuid                = inputMsg.uuid,
                        ...
                        qCritical()
                            << "Achieved VPNHandshake(Request) command but Server is impossible to create.";
                }
                qInfo() << "Request server end";
            } else if (inputMsg.vpnType == VPNType::PROXY) {
                qInfo() << "Request proxy 1";
                if (!node.vpnConnectorManager->CheckVPNHandshakeAccess(identifier, 8))
                    return;
                ...
                if (!inputMsg.networkIdentifiersToIgnore.contains(
                    ...
                    VPNFunctionsResult output;
                    if (node.vpnConnectorManager
                            ->networkCallback(inputMsg, mb.sender_id, VPNFunctionType::CHECK_PROXY, output)) {
                        ...
                        outputMsg.lockedChainIndex.insert(
                            output.blockedChainIndexes.begin(),
                            output.blockedChainIndexes.end());
                        ...
                        node.vpnConnectorManager->vpnHandhakeCacheInProccess->emplaceBack(
                            VPNConnectorManager::VPNHandhakeCache { .uuid                = inputMsg.uuid,
                                                                    ...
                                                                    .proxyResponseMessageID = messageId });
                        ...
                        qCritical()
                            << "Achieved VPNHandshake(Request) command but Proxy is impossible to create.";
                }
        ...
        if (status == MessageStatus::Response) {
            ...
            if (inputMsg.vpnType == VPNType::SERVER || inputMsg.vpnType == VPNType::PROXY) {
                if (node.vpnConnectorManager->vpnIsClient) {
                    QTimer::singleShot(
                        2000,
                        this,
                        [this, inputMsg, senderID = mb.sender_id, identifier]() mutable {
                            ...
                            if (node.vpnConnectorManager->networkCallback(
                                ...
                                if (node.vpnConnectorManager->networkCallback(
                                    ...
                                    for (auto it = m_reconnectionsToIdentifier.begin();
                                         it != m_reconnectionsToIdentifier.end();
                                         ++it) {
                                    ...
                                    node.vpnConnectorManager->vpnUuidToVPNWorkers->emplace(
                                        inputMsg.uuid,
                                        VPNConnectorManager::VPNWorkers {
                                            .uuid             = inputMsg.uuid,
                                    ...
                                    node.vpnConnectorManager->vpnConnectedType = VPNType::CLIENT;
                    ...
                    for (auto it = vpnHandhakeCacheInProccessLocked->begin();
                        ...
                        if (it->chainIndex == inputMsg.resultChainIndex && it->uuid == inputMsg.uuid) {
                            ...
                            outputMsg.publicKeyFile =
                                node.vpnConnectorManager
                                    ->vpnFileAddedHash[node.vpnConnectorManager->vpnUuidToVPNWorkers->size()];
                            outputMsg.vpnType          = VPNType::PROXY;
                            outputMsg.resultChainIndex = inputMsg.resultChainIndex;
                            outputMsg.uuid             = inputMsg.uuid;
                            outputMsg.publicIP =
                                node.vpnConnectorManager->vpnInitPublicIPAndCountry.first.toStdString();
                                 ...
                                 nextIdent      = it->nextIdentifier]() mutable {
                                    ...
                                    if (node.vpnConnectorManager->networkCallback(
                                        ...
                                        for (auto it = m_reconnectionsToIdentifier.begin();
                                             it != m_reconnectionsToIdentifier.end();
                                             ++it) {
                                            qInfo() << "Identifiers req&next" << it->first.ip
                                                    << it->first.port << it->second;
                                            if (it->second == requesterIdentifier) {
                                                requesterIP   = it->first.ip;
                                                requesterPort = it->first.port;
                                            } else if (it->second == nextIdentifier) {
                                                nextIP   = it->first.ip;
                                                nextPort = it->first.port;
                                            }
                                        ...
                                        node.vpnConnectorManager->vpnUuidToVPNWorkers->emplace(
                                            ...
                                            VPNConnectorManager::VPNWorkers {
                                                .uuid                 = inputMsg.uuid,
                                                .chainIndex           = inputMsg.resultChainIndex,
                                                .requesterIdentifier  = requesterIdent,
                                                .requesterIP          = requesterIP,
                                                .requesterPort        = requesterPort,
                                                .nextIdentifier       = nextIdent,
                                                .nextIP               = nextIP,
                                                .nextPort             = nextPort,
                                                .lastUpdateRequsterTS = QDateTime::currentMSecsSinceEpoch(),
                                                .lastUpdateNextTS     = QDateTime::currentMSecsSinceEpoch(),
                                                .lastSendedNextTS = QDateTime::currentMSecsSinceEpoch() });
                                        node.vpnConnectorManager->vpnConnectedType = VPNType::PROXY;
                                    } else {
            ...
            if (inputMsg.vpnType == VPNType::SERVER) {
                ...
                bool canProccess = false;
                {
                    ...
                    for (auto &it : *vpnHandhakeCacheInProccessLocked) {
                        qInfo() << "vpnHandhakeCacheInProccess" << it.chainIndex << it.uuid
                                << it.requesterIdentifier;
                        if (it.chainIndex == inputMsg.resultChainIndex && it.uuid == inputMsg.uuid
                            && it.requesterIdentifier == identifier) {
                ...
                if (canProccess) {
                    ...
                    // open VPN server
                    if (node.vpnConnectorManager
                            ->networkCallback(inputMsg, mb.sender_id, VPNFunctionType::SET_SERVER, output)) {
                        ...
                        if (!node.vpnConnectorManager->networkCallback(
                                ...
                                output)) {
                            qCritical()
                                << "Achieved VPNConnection(Request) SERVER command but cannot get Public IP";
                            break;
                        }
...

                        {
                            ...
                            node.vpnConnectorManager->vpnUuidToVPNWorkers->emplace(
                                inputMsg.uuid,
                                VPNConnectorManager::VPNWorkers { .uuid       = inputMsg.uuid,
                                                                  .chainIndex = inputMsg.resultChainIndex,
                                                                  .requesterIdentifier = identifier,
                                                                  .requesterIP         = ip,
                                                                  .requesterPort       = port,
                            ...
                            node.vpnConnectorManager->vpnConnectedType = VPNType::SERVER;
                        }

                        {
                            ...
                            for (auto it = vpnHandhakeCacheInProccessLocked->begin();
                                 it != vpnHandhakeCacheInProccessLocked->end();
                                 ++it) {
                                if (it->chainIndex == inputMsg.resultChainIndex
                                    && it->uuid == inputMsg.uuid) {
                ...
                qInfo() << "MUTEX 13";
                auto vpnHandhakeCacheInProccessLocked = *node.vpnConnectorManager->vpnHandhakeCacheInProccess;
                for (auto &it : *vpnHandhakeCacheInProccessLocked) {
                    ...
                    if (it.chainIndex == inputMsg.resultChainIndex && it.uuid == inputMsg.uuid
                        ...
                        it.localIPForSetup =
                            "101." + std::to_string(inputMsg.proxyCounter) + chainIndexStr + ".0.1";
    ...
    case MessageType::VPNDisconnect: {
        ...
        if (node.vpnConnectorManager->vpnConnectedType.has_value()
            && node.vpnConnectorManager->vpnConnectedType.value() != VPNType::SERVER) {
            qInfo() << "MUTEX 14";
            auto res = node.vpnConnectorManager->vpnUuidToVPNWorkers->find(inputMsg.uuid);
            if (res != node.vpnConnectorManager->vpnUuidToVPNWorkers->end()) {
        ...
        QTimer::singleShot(200, this, [this, inputMsg, senderID = mb.sender_id]() mutable {
            VPNFunctionsResult output;
            node.vpnConnectorManager
                ->networkCallback(inputMsg, senderID, VPNFunctionType::DISCONNECT, output);
        });
