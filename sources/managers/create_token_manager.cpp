...
void CreateTokenManager::checkIsContract(const QString &pathToFile) {
    if (isContract(pathToFile)) {
        ...
        if (jsonDoc.isObject()) {
            ...
            rowRow.insert({"count_coins", QString::number(jsonObj[Token::Fields::count.c_str()].toInt()).toStdString()});
...
bool CreateTokenManager::checkJsonObjectHasTokenFields(const QJsonObject &jsonObj) {
