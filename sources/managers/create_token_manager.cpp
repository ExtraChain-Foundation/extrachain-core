...
void CreateTokenManager::createToken(const std::string &tokenCount,
  ...
  if (isDbOpen) {
    ...
    rowRow.insert({"owner", actorId.toStdString()});
    rowRow.insert({"color", color});
...
void CreateTokenManager::checkIsContract(const QString &pathToFile) {
  if (isContract(pathToFile)) {
    ...
    if (jsonDoc.isObject()) {
      ...
      auto owner = jsonObj["owner"].toString();

      DBRow rowRow;
      rowRow.insert({"name", jsonObj["name"].toString().toStdString()});
      rowRow.insert({"symbol", jsonObj["symbol"].toString().toStdString()});
      rowRow.insert({"count_coins",
                     QString::number(jsonObj["count"].toInt()).toStdString()});
      rowRow.insert({"owner", jsonObj["owner"].toString().toStdString()});
      rowRow.insert({"color", jsonObj["color"].toString().toStdString()});
      rowRow.insert({"url", std::string()});
...
bool CreateTokenManager::checkJsonObjectHasTokenFields(
  ...
  QStringList requiredFields = {"name",  "symbol", "count",
                                "owner", "color",  "url"};
