...
void ThothManager::flush_pending_records() {
    ...
    auto reg_json = Json::serialize(reg);
...
void ThothManager::network_thoth_record(const ActorId& owner_id, const std::string& file_id, const DbRow& row) {
    ...
    auto security_key = Dfs::DataSecurityActor { .sender_id = ActorId(actor_it->second),
                                                  .receiver_id = node->network_id() };
    auto current = node->dfs()->read_vector_row(owner_id,
                                                 file_id,
                                                 id_it->second,
                                                 security_key,
                                                 Dfs::FileType::Dictionary);
...
void ThothManager::set_device_token(const std::string& token) {
    ...
    // Same token still needs the startup reconciliation: it repairs a stale remote value.
    if (token == ios_token_) {
        persist_device_tokens();
        flush_pending_records();
        reconcile_after_token_change();
        return;
    }
    ios_token_ = token;
    persist_device_tokens();
...
// read_chats() often runs before the platform delivers the push token; its reconcile
// no-ops on an empty token and is never retried. Re-run it with the already-loaded list.
void ThothManager::reconcile_after_token_change() {
    if (node->chat_manager() && !node->chat_manager()->chats().empty()) {
        reconcile_tokens_for_chats(node->chat_manager()->chats());
    }
}
...
void ThothManager::reconcile_tokens_for_chats(const std::vector<Chat::Chat>& chats) {
    ...
    // The first ready chat list after startup (or after a token/device-id change) rewrites
    // every key. This wins a stale file-level sync race without a tombstone for the old token.
    if (!force_registry_write_ && ios_token_ == reconciled_token_ && chats.size() == reconciled_chats_count_) {
        return;
    }
    reconciled_token_       = ios_token_;
    ...
    for (const auto& chat : chats) {
        ...
        last_reconciled_records_.push_back({ .owner_id = chat.owner_id, .file_id = chat.file_id, .custom = custom });
        enqueue_thoth_record(chat.owner_id, chat.file_id, custom);
    }
    flush_pending_records();
