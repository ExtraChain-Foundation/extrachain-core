...
ExtraChainNode::ExtraChainNode(const bool isApp) : isApp(isApp) {
  ...
  m_connectionsManager = new ConnectionsManager(
      "12.12.12.12", "1212", actorIndex()->firstId().toByteArray(), this);
...
bool ExtraChainNode::createNewNetwork(const QString &email,
  ...
  if (QDir("keystore/profile").isEmpty()) {
    ...
    auto consoleHash =
        Utils::calcHash(email.toStdString() + password.toStdString());
    auto first =
        m_accountController->createProfile(consoleHash, ActorType::DAppMaster);
  ...
  if (m_blockchain->getRecords() <= 0) {
    ...
    //  emit generateSmartContract(tokenCount.toLatin1(), tokenName.toUtf8(),
    //  first.id().toByteArray(),
    //                             tokenColor.toLatin1());
    ...
    //        // m_dfs->save(DfsStruct::DfsSave::Static, "usernames", "",
    //        DfsStruct::Type::Service);
  }
...
//    &ResolveManager::resolveMessage); connect(resolveManager,
//    &ResolveManager::coinRequest, this, &ExtraChainNode::coinResponse);
//    connect(dfs->networkManager(), &DfsNetworkManager::newMessage,
//    resolveManager,
//            &ResolveManager::resolveMessage);
// TODO: move
//    connect(resolveManager, &ResolveManager::sendMsg, m_networkManager,
//    &networkManager::sendMessage);

// connect(this, &ExtraChainNode::sendMsg, m_resolveManager,
// &ResolveManager::registrateMsg); connect(m_txManager,
// &TransactionManager::SendBlock, m_resolveManager,
// &ResolveManager::registrateMsg); connect(m_blockchain,
// &Blockchain::sendMessage, m_resolveManager, &ResolveManager::registrateMsg);
//    connect(dfs, &Dfs::newSender, resolveManager,
//    &ResolveManager::registrateMsg);
// }

void ExtraChainNode::connectTxManager() {}

Blockchain *ExtraChainNode::blockchain() { return m_blockchain; }

NetworkManager *ExtraChainNode::network() { return m_networkManager; }

std::expected<Transaction, TransactionError>
ExtraChainNode::createTransaction(Transaction tx) {
  if (tx.getAmount() == 0) {
  ...
  if (tx.isEmpty() && !tx.isBurn()) {
    qWarning() << fmt::format("Can not create tx:[{}]. Transaction is empty",
                              tx.toStdString());
  ...
  if (actor->empty()) {
    qWarning() << fmt::format("Can not create tx:[{}]. There no current user",
                              tx.toStdString());
    return std::unexpected(TransactionError::NoCurrentUser);
  }

  qWarning() << fmt::format("Attempting to create tx:[{}] from user [{}]",
                            tx.toStdString(), actor->id().toStdString());
  ...
  // 3) sign transaction
  tx.sign(actor);
  qDebug() << "[Transaction] Send" << tx.getAmountDec() << "to"
           << tx.getReceiver();
...
std::expected<Transaction, TransactionError>
ExtraChainNode::createTransaction(ActorId receiver, BigNumberFloat amount,
                                  ActorId token) {
  auto actor = m_accountController->currentWallet();
...
bool ExtraChainNode::importUser(const std::string &data,
  ...
  auto profileBytesEncrypted = QByteArray::fromStdString(
      SecretKey::encryptWithPassword(profileBytes.toStdString(), hash));

  Q_UNUSED(extrachainVersion)
  Q_UNUSED(date)

  QString privateProfile =
      "keystore/" + profile["main"].toString() + ".profile";
  ...
  m_accountController->addToProfileList(
      profile["main"].toString().toStdString());

  return true;
}

std::expected<Transaction, TransactionError>
ExtraChainNode::createTransactionFrom(ActorId sender, ActorId receiver,
                                      BigNumberFloat amount, ActorId token) {
  ...
  if (receiver.isZero() && !amount.isEmpty()) {
    if (!actor->empty()) {
      ...
      BigNumber lastBlockId = m_blockchain->getLastRealBlock().getIndex();
      if (lastBlockId.isEmpty()) {
        qWarning() << QString("Can not create tx: [%1]. There is no last block "
                              "in blockchain")
                          .arg(tx.toString())
                          .toStdString();
        return std::unexpected(TransactionError::NoLastBlock);
      ...
      // 2) check coin availability
      //                if (blockchain()->getUserBalance(actor.id(),
      //                tx.getToken()) < tx.getAmount()) {
      //                    qDebug() << QString("Warning: can not create
      //                    tx:[%1]. There is not enough "
      ...
      tx.sign(actor);
      qDebug() << "[Transaction] Send tx" << tx.getAmountDec() << "to"
               << tx.getReceiver();
...
std::expected<Transaction, TransactionError>
ExtraChainNode::createFarmingTransaction(ActorId sender,
...
std::string
ExtraChainNode::transactionErrorDescription(const TransactionError &error) {
  switch (error) {
  ...
  case TransactionError::InsufficientFunds:
    return "Can not create transaction. There is not enough coins/tokens in "
           "wallet.";
...
void ExtraChainNode::getAllActorsTimerCall() {
  if (m_accountController->count() > 0 &&
      m_networkManager->connections().length() > 0) {
...
void ExtraChainNode::notificationToken(QString os, QString actorId,
                                       QString token) {
  ...
  // },
  //                                            { "token",
  //                                            mainKey.encrypt(token.toStdString(),
  //                                            publicKey)
  //                                            }, { "os",
  //                                            mainKey.encrypt(os.toStdString(),
  //                                            publicKey) } };

  // TODONEW emit sendMsg(Serialization::serializeMap(map),
  // Messages::GeneralRequest::Notification);
}
...
void ExtraChainNode::handleCountMessageReceived(BigNumber count) {
  ...
  if (m_connectionsManager->getActiveConnection().size() ==
      resiveCounts.size()) {
    BigNumber sum = 0;
    ...
    // Tu - total network uptime. Blocks are formed every 2 seconds. Therefore,
    // by taking the total number
    //      of blocks in the network and multiplying them by 2, it is possible
    //      to determine how many seconds the network has been online.
    ...
    blockCount =
        m_connectionsManager->getActivityScore(Connection{ip, port, true}) /
...
void ExtraChainNode::connectActorIndex() {
  // connect(m_actorIndex, &ActorIndex::sendMessage, m_resolveManager,
  // &ResolveManager::registrateMsg);
}

void ExtraChainNode::dfsConnection() {
  // init dfs for user
  connect(m_networkManager, &NetworkManager::addFragSignal, m_dfs,
          &DfsController::threadAddFragment);
  connect(m_networkManager, &NetworkManager::fetchFragment, m_dfs,
          &DfsController::fetchFragment);
  connect(this, &ExtraChainNode::ready, m_networkManager,
          &NetworkManager::startNetwork);
  // connect(this, &ExtraChainNode::ready, m_dfs, &Dfs::startDFS);
  // connect(m_accountController, &AccountController::initDfs, m_dfs,
  // &Dfs::initMyLocalStorage); connect(m_actorIndex, &ActorIndex::initDfs,
  // m_dfs, &Dfs::initUser);
  //    connect(chatManger, &ChatManager::sendDataToBlockhainFromChatManager,
  //    dfs, &Dfs::savedNewData); connect(networkManager,
  //    &NetworkManager::newDfsSocket, dfsNetworkManager,
...
void ExtraChainNode::connectSignals() {
  ...
  connect(m_networkManager, &NetworkManager::newSocket, m_blockchain,
          &Blockchain::updateBlockchain);
  connect(m_networkManager, &NetworkManager::newSocket,
          [this]() { m_dfs->requestSync(); });
  connect(m_networkManager, &NetworkManager::newSocket, [this]() {
...
void ExtraChainNode::prepareFolders() {
  ...
  QDir().mkpath(DataStorage::BLOCKCHAIN_INDEX + "/" +
...
void ExtraChainNode::calculateBlockCount() {
  ...
  m_networkManager->send_message(msg, MessageType::RequestBlockCount,
...
ActorIndex *ExtraChainNode::actorIndex() const { return m_actorIndex; }

DfsController *ExtraChainNode::dfs() const { return m_dfs; }

TransactionManager *ExtraChainNode::txManager() const { return m_txManager; }

DataMiningManager *ExtraChainNode::dataMiningManager() const { return m_dmm; }
