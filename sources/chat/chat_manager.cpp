...
std::expected<Dfs::DirRow, ChatError> ChatManager::create_chat_folders() {
    auto existing = find_chat_folders_row();
    if (existing.has_value()) {
        return existing.value();
    }
    auto chat_actor_id  = current_chat_actor_id();
    auto security_actor = Dfs::DataSecuritySelf { .my_actor = chat_actor_id };
    auto store_res      = node->dfs()->store_dictionary(chat_actor_id,
                                                    chat_actor_id,
                                                    CHAT_FOLDERS,
                                                    Dfs::DataSecurity::Self,
                                                    security_actor);
...
std::optional<Dfs::DirRow> ChatManager::find_chat_folders_row() {
    auto chat_actor_id = current_chat_actor_id();
...
std::expected<Chat::ChatFolder, ChatError> ChatManager::create_chat_folder(const std::string& name) {
    auto folder_row = create_chat_folders();
    ...
    auto res = node->dfs()->dictionary_set_value(chat_actor_id, folder_row->file_id, folder.id, value,
                                                  chat_actor_id, security_actor);
    if (!res) {
        return std::unexpected(ChatError::Unknown);
    }
...
bool ChatManager::update_chat_folder(const Chat::ChatFolder& folder) {
    if (folder.id.empty()) {
        return false;
    }
    auto folder_row = find_chat_folders_row();
    if (!folder_row.has_value()) {
        return false;
    }
    auto chat_actor_id  = current_chat_actor_id();
    auto security_actor = Dfs::DataSecuritySelf { .my_actor = chat_actor_id };
    auto value          = Json::serialize(folder);
    return node->dfs()->dictionary_set_value(chat_actor_id, folder_row->file_id, folder.id, value,
                                              chat_actor_id, security_actor);
}

bool ChatManager::remove_chat_folder(const std::string& folder_id) {
    auto folder_row = find_chat_folders_row();
    if (!folder_row.has_value()) {
        return false;
    }
    auto chat_actor_id = current_chat_actor_id();
    return node->dfs()->dictionary_remove_value(chat_actor_id, folder_row->file_id, folder_id, chat_actor_id);
}

std::expected<std::vector<Chat::ChatFolder>, ChatError> ChatManager::read_chat_folders() {
    auto folder_row = find_chat_folders_row();
    if (!folder_row.has_value()) {
        return std::vector<Chat::ChatFolder> {};
    }
    auto chat_actor_id  = current_chat_actor_id();
    auto security_actor = Dfs::DataSecuritySelf { .my_actor = chat_actor_id };
    auto rows = node->dfs()->read_dictionary_rows(chat_actor_id, folder_row->file_id, security_actor);
    if (!rows.has_value()) {
        return std::vector<Chat::ChatFolder> {};
    }
    std::vector<Chat::ChatFolder> folders;
    folders.reserve(rows->size());
    for (const auto& [key, value] : rows.value()) {
