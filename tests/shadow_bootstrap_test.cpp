...
int main(int argc, char** argv) {
    ...
    Transaction invalid_spend;
    invalid_spend.set_sender(actor.id());
    invalid_spend.set_receiver(receiver.id());
    invalid_spend.set_token(actor.id());
    invalid_spend.set_type(TransactionType::Regular);
    invalid_spend.set_amount(BigNumberFloat(1000));
    invalid_spend.set_section(SectionId(30));
    invalid_spend.set_timestamp(100);
    TEST_REQUIRE(invalid_spend.sign(actor));
    TEST_REQUIRE(dag->save_transaction(invalid_spend));
    dag->report_state_inconsistency(SectionId(30), "negative-balance-transition", "bootstrap-test");
    TEST_REQUIRE(!dag->prepare_shadow_activation().has_value());
    const auto invalid_prefix = dag->prepare_shadow_activation(SectionId(40));
    TEST_REQUIRE(!invalid_prefix.has_value());
    TEST_REQUIRE_EQ(invalid_prefix.error(), ConsensusError::InvalidRoot);
