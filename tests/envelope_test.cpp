// Unknown origins must be resolved and verified before application dispatch.
...
int main() {
    ...
    auto        connection = node->network()->custom_message_event().subscribe(
        [&delivered](const NetworkPackageStorage &, const CustomMessage &) {
            ++delivered;
        });

    const auto envelope =
        [](const Actor<KeyPrivate> &signer, const ActorId &claimed_origin, bool corrupt_signature) {
    ...
    node->network()->message_received(envelope(peer, ActorId(), false), "127.0.0.1", "peer-node");
    TEST_REQUIRE_EQ(delivered, std::size_t(1));

    // An unknown origin cannot dispatch application data.
    node->network()->message_received(envelope(impostor, impostor.id(), false), "127.0.0.1", "peer-node");
    TEST_REQUIRE_EQ(delivered, std::size_t(1));

    auto forged_actor = impostor.to_public();
    forged_actor.set_id(peer.id());
    TEST_REQUIRE(!node->actor_index()->save_actor(forged_actor).has_value());
    for (std::size_t length = 0; length < 64; ++length) {
        node->network()->message_received(std::string(length, 'x'), "127.0.0.1", "peer-node");
    }
    TEST_REQUIRE_EQ(delivered, std::size_t(1));

    const auto        carried         = impostor.to_public();
    const MessageBody actor_body      = make_init_message(MessagePack::serialize(carried),
                                                     SendMode::Focused,
                                                     MessageType::Actor,
                                                     ...
                                                     std::string(),
                                                     "peer-node");
    const auto        actor_signature = peer.key().sign(ByteArray(actor_body.calculate_hash()).toBytes());
    TEST_REQUIRE(actor_signature.has_value());
    node->network()->message_received(actor_body.serialize() + ByteArray(actor_signature.value()).toString(),
                                      "127.0.0.1",
                                      "peer-node");
