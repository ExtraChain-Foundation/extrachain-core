...
int main(int argc, char *argv[]) {
    ...
    for (SectionId start(1); start + SectionId(CTRL_MOD - 1) <= closed_tip; start += CTRL_MOD) {
        ...
        if (!stored.has_value()) {
            ctrl_missing++;
            if (ctrl_missing <= 5)
                std::printf("  [ctrl] missing at %s\n", end.to_string().c_str());
        } else if (stored->control != last_hash) {
            ctrl_mismatch++;
            if (ctrl_mismatch <= 5)
                std::printf("  [ctrl] mismatch at %s\n", end.to_string().c_str());
        }
    }
    ...
    for (SectionId i(0); i <= closed_tip; i += CTRL_MOD) {
        ...
        if (from_sec != from_idx) {
            ci_mismatch++;
            if (ci_mismatch <= 5)
                std::printf("  [index] mismatch at %s\n", i.to_string().c_str());
        }
    }
