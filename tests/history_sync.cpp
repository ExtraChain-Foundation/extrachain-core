...
int main() {
    ...
    const auto cache_before_failed_reset = dag.cache().read_cached_balances();
    TEST_REQUIRE(cache_probe.open());
    TEST_REQUIRE(
        cache_probe.query("CREATE TRIGGER reject_cache_reset BEFORE DELETE ON balance_cache "
    ...
    TEST_REQUIRE(std::filesystem::exists(ChainConst::PACK_REPLAY_REQUIRED));
    TEST_REQUIRE_EQ(node->dag()->cache().read_cached_balances(), cache_before_failed_reset);
    TEST_REQUIRE(cache_probe.open());
    TEST_REQUIRE(cache_probe.query("DROP TRIGGER reject_cache_reset"));
    cache_probe.close();
    node->cleanUp();
    node.reset();
    node = std::make_unique<ExtraChain::Core::ExtraChainNode>(false, false, 0);
    ...
    const auto recovered_cache = node->dag()->cache().read_cached_balances();
    TEST_REQUIRE(recovered_cache.first >= SectionId(0));
    node->cleanUp();
    node.reset();
    node = std::make_unique<ExtraChain::Core::ExtraChainNode>(false, false, 0);
    node->process();
    TEST_REQUIRE_EQ(node->dag()->cache().read_cached_balances(), recovered_cache);
    TEST_REQUIRE(!std::filesystem::exists(ChainConst::PACK_REPLAY_REQUIRED));
    node->cleanUp();
    node.reset();
