...
int main(int argc, char *argv[]) {
    ...
    auto *wrapper = new ExtraChainNodeWrapper(&app, false, false, 17599);
    wrapper->init(false);
    ...
    // for every control-aligned section across the whole chain.
    {
        ...
        for (SectionId i(0); i <= cur; i += 20) {
            ...
            if (sec.has_value() && sec->control.has_value()) from_section = sec->control.value();
            auto from_index = node->dag()->read_control(i); // index-backed
            std::optional<std::string> idx;
            if (from_index.has_value()) idx = from_index->control;
            checked++;
            if (from_section != idx) {
                mism++;
                if (mism <= 5)
